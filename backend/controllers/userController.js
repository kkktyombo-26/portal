...
// PUT /api/users/:id — Pastor can edit anyone; others can only edit themselves
exports.updateUser = async (req, res, next) => {
  try {
    ...
    const [rows] = await db.execute('SELECT id, full_name, email, role, phone, is_active FROM users WHERE id = ?', [targetId]);
    res.json({ success: true, data: rows[0], message: 'User updated.' });
  } catch (err) { next(err); }
};

// DELETE /api/users/:id — Pastor only
